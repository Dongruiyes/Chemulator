...
class Frame_mw(wx.Frame):
    def __init__(self):
        ...
        self.图文按钮L2 = lib_gb_GradientButton(self.启动窗口, size=(100, 32), pos=(336, 439), bitmap=None, label='计算结果',
                                            name='gradientbutton')
        ...
        self.标签6 = wx_StaticTextL(self.启动窗口, size=(192, 24), pos=(728, 449), label='设置结果小数位数：', name='staticText',
                                  style=256)
        ...
        编辑框8_字体 = wx.Font(12, 74, 90, 400, False, 'Microsoft YaHei UI', -1)
        self.编辑框8.SetFont(编辑框8_字体)
        self.编辑框8.SetForegroundColour((128, 0, 0, 255))
    ...
    def on_close(self, event):
        # 检查是否存在 Batch 方法正在执行
        if 'Batch' in self.__dir__() or 'main' in self.__dir__():
            self.Destroy()  # 直接退出程序
        else:
            dlg = wx.MessageDialog(self, "是否退出程序？", "摩尔浓度换算", wx.YES_NO | wx.ICON_QUESTION)
            result = dlg.ShowModal()
